<template>
  <div class="about">
    <h1>This is an about page</h1>
    <div class="container">

    <h3>
      A simple task app made by Fahad with the javascript framework called Vue.\
      <br />
      This is my first project made with Vue framework! Some unnecessary text
      here to make this page big LOL! <br />
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Illum temporibus
      consequuntur alias fugiat tempore rem. Et vero deleniti libero optio
      dignissimos odio sapiente, aut eum, dolorem laborum ducimus numquam minus
      fugit mollitia quidem iusto omnis exercitationem nulla hic accusantium!
      Minima nostrum magnam excepturi natus cumque, iure totam molestias quaerat
      explicabo sit eaque nobis repellat in ipsa. Dolore facilis inventore,
      veniam tenetur culpa ipsa illum corporis dolorem quisquam asperiores illo
      totam molestiae veritatis sed ipsam, voluptatibus in. Error nisi officiis
      porro molestiae sed aperiam vel, eius ratione nobis quia quas inventore
      eligendi dolores doloremque eum quae, minus qui soluta! Tenetur,
      voluptatem.
    </h3>
  </div>
    </div>
</template>
